...
def generate_equipment_master(download_path, supabase):

    # -------------------------------
    # FETCH DATA
    # -------------------------------
    res = supabase.table("v_equipment_master") \
        .select("*") \
        .order("machine_name") \
        .execute()
    ...
    def clean_text(value, max_len=35):
        ...
        return text[:max_len]  # limit length (VERY IMPORTANT)
    ...
    for sheet_idx in range(total_sheets):
        ...
        # KEEP ORIGINAL COLUMN WIDTHS (IMPORTANT)
        for col in temp_ws.column_dimensions:
            ws.column_dimensions[col].width = temp_ws.column_dimensions[col].width
        # 🔥 FIX: Reduce C column width
        ws.column_dimensions['B'].width = 18   # slightly reduced
        ws.column_dimensions['C'].width = 25   # slightly increased
    ...
    file_path = os.path.join(download_path, "equipment_master.xlsx")
    final_wb.save(file_path)

    print(f"\n✅ FINAL CLEAN TEMPLATE REPORT ({total_sheets} sheets)")
